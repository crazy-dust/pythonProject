...
# 是否正确使用 * 和 ** 解包？
# 是否避免滥用 global？
# 高阶函数是否使用 Callable 标注？
# Lambda 是否只用于简单表达式？
# 闭包是否确实引用了外层变量？
# 装饰器是否返回原函数结果？
# 装饰器是否使用 @wraps？
# /*  前者是/之前的是位置参数，后者是*之后的是关键字参数
# 今天必须记住
...
# 闭包会保存外层函数环境
# 装饰器本质是接收函数并返回新函数
# @decorator 是 function = decorator(function) 的语法糖
# 生产装饰器要使用 functools.wraps
#
# 第四天最核心的理解是：
# Python 函数不是类中的附属方法，而是可以自由组合、传递和包装的一等对象。